    ...
    <article key={id} className="flex items-center justify-between w-full max-w-screen-sm gap-3 p-5 bg-white border border-slate-300">
      ...
      <div className="flex shrink-0 gap-x-6">
        <Menu as="div" className="relative flex-none" >
            <Menu.Button className="-m-2.5 block p-2.5 text-gray-500 hover:text-gray-900" onClick={()=>setVisible(!visible)} >
                <span className="sr-only">opciones</span>
                <EllipsisVerticalIcon className="h-9 w-9" aria-hidden="true"  />
            </Menu.Button>
            
            <Transition show={visible} as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95"
            ...
            <Menu.Items 
                ...
                <Menu>
                    <button type='button' className='block px-3 py-1 text-sm leading-6 text-gray-900 hover:text-violet-500'
                    onClick={() =>  {handleEdit(), setVisible(false)}}>
                        Editar Tarea
                    </button>
                </Menu>
